...
async function saveQuote() {
  ...
  showLoadingOverlay('Guardando cotización...');
  const num = await State.nextQuoNum();
  ...
  const quo = {
    ...
    items, subtotal: sub, tax, discount: disc,
    total:  Math.max(0, sub + tax - disc),
    notes:  document.getElementById('quo-notes')?.value || '',
    status: 'quote', type: 'quote'
  };

  const saved = await State.addQuote(quo);
  hideLoadingOverlay();

  if (saved) {
    toast('✅ Cotización ' + saved.number + ' guardada');
    clearQuoteForm();
  } else {
    toast('Error al guardar la cotización', 'error');
  }
}
